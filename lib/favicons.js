...
function generateIcons (loader, imageFileStream, pathPrefix, query, callback) {
  ...
  favicons(imageFileStream, {
    ...
    var html = result.html.map(function (entry) {
      ...
      if (entry.indexOf("content=") !== -1) {
        return entry.replace(/ content="([^"]+)"/g, (match, value) => {
          var prefixed = publicPath + pathPrefix + value;
          return (result.files.indexOf(prefixed) !== -1) ? ("content=\"" + prefixed + "\"") : match;
        });
      }
    });
    var loaderResult = {
